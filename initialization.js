...
console.log("WALLET_ADDRESS:", process.env.WALLET_ADDRESS || "❌ missing");
console.log("ARBITRAGE_CONTRACT_ADDRESS:", process.env.ARBITRAGE_CONTRACT_ADDRESS || "❌ missing");
...
// ✅ ABIs (minimal factory/router interfaces)
const factoryABI = [
  {
    constant: true,
    inputs: [
...
const uFactory = new web3.eth.Contract(factoryABI, UNISWAP_FACTORY_ADDRESS);
const uRouter = new web3.eth.Contract(routerABI, UNISWAP_ROUTER_ADDRESS);

const sFactory = new web3.eth.Contract(factoryABI, SUSHISWAP_FACTORY_ADDRESS);
const sRouter = new web3.eth.Contract(routerABI, SUSHISWAP_ROUTER_ADDRESS);

const qFactory = new web3.eth.Contract(factoryABI, QUICKSWAP_FACTORY_ADDRESS);
const qRouter = new web3.eth.Contract(routerABI, QUICKSWAP_ROUTER_ADDRESS);
...
if (arbitrageAddress && arbitrageAddress !== "") {
  ...
  console.warn("⚠️ ARBITRAGE_CONTRACT_ADDRESS is missing. Arbitrage contract not loaded.");
}
